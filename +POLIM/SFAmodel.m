classdef SFAmodel
    %SFAMODEL object that contains the data to calculate the 2D sinfle
    ...
    methods
        function obj = SFAmodel(mex, pex, mf, pf, X, epsilon, ex_ang_rad, em_ang_rad) % object constructor
        ...
        function [I_ex_em, exAng, emAng] = getPortrait(obj)
            % get number of excitation angles
            nExAng  = length(unique(obj.ex_ang_rad));
            % get number of emission angles
            nEmAng  = length(unique(obj.em_ang_rad));
            
            % get portrait
            I_ex_em = reshape(obj.intensity,nExAng,nEmAng);
            % get reordered angles
            exAngPlot = reshape(obj.ex_ang_rad,nExAng,nEmAng);
            emAngPlot = reshape(obj.em_ang_rad,nExAng,nEmAng);
            
            % now check how the portrait is built
            if sum(diff( exAngPlot(:,1))) > 0
                %then excitation angles change first while em is constant
                assert(sum(diff( emAngPlot(:,1))) == 0, 'Problems, em angles change unexpectedly' )
                exAng = exAngPlot(:,1)';
                emAng = emAngPlot(1,:);
            elseif sum(diff( emAngPlot(:,1))) > 0
                %then emission angles change first while ex is constant
                assert(sum(diff( exAngPlot(:,1))) == 0, 'Problems, ex angles change unexpectedly' )
                exAng = exAngPlot(1,:);
                emAng = emAngPlot(:,1)';
                % transpose is needed in this case due to the way reshape works, it
                % actually does not know which angle is changing
                I_ex_em = I_ex_em';
            else
            error('Unexpected in polarization angles')
            end
